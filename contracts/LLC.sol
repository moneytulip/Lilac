 ...
 */
contract LLC is ERC20Permit, Pausable, SimpleGovernance {
    ...
    /**
     * @notice Initializes LLC token with specified governance address and recipients. For vesting
    ...
    ) ERC20("Lilac", "LLC") ERC20Permit("Lilac") {
        require(governance_ != address(0), "LLC: governance cannot be empty");
        require(
            vestingContractTarget_ != address(0),
            "LLC: vesting contract target cannot be empty"
        );
        require(
            pausePeriod_ > 0 && pausePeriod_ <= 52 weeks,
            "LLC: pausePeriod must be in between 0 and 52 weeks"
        );
    ...
    /**
     * @notice Deploys a clone of the vesting contract for the given recipient.
     * This will potentially emulate the details about vesting and the token
     * release schedule found on https://docs.saddle.finance
        ...
        returns (address)
    {
        require(
            ...
            "LLC: duration for vesting cannot be 0"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || allowedTransferee[from], "LLC: paused");
        require(to != address(this), "LLC: invalid recipient");
    }
    ...
    function rescueTokens(
        ...
        require(to != address(0), "LLC: invalid recipient");
        ...
        if (token == IERC20(address(0))) {
            ...
            require(balance > 0, "LLC: trying to send 0 ETH");
            // slither-disable-next-line arbitrary-send
            (bool success, ) = to.call{value: balance}("");
            require(success, "LLC: ETH transfer failed");
        } else {
            // any other erc20
            ...
            balance = balance == 0
                ? totalBalance
                : Math.min(totalBalance, balance);
            require(balance > 0, "LLC: trying to send 0 balance");
